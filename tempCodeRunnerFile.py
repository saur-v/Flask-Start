...
def about_you():
    if request.method=="POST" and len(request.form["name"])>0:
        ...
        df.to_csv('static/data.csv', mode='a', index=False, header=True)  
...
def user_data():
    if request.method=="POST":
        df = pandas.read_csv('static/data.csv', names=['name','email','contact'])
...
about_data = { 'name':['saurav'],
                        'email': ['rishabhagrawal3105@gmail.com'],
                    'contact':[ '7851946847']  
        }
